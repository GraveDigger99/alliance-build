(() => {
    ...
    da.init();
    let calculateBtn = document.querySelector(".investement-calculator__button");
    if (calculateBtn) {
        const getVal = () => {
            let totalPrice = Number(document.getElementById("total-price").value);
            let percentageRate = Number(document.getElementById("percentage-rate").value) / 100;
            let monthlyPercentRate = percentageRate / 12;
            let numberOfYears = Number(document.getElementById("number-of-years").value);
            let numberOfMonth = numberOfYears * 12;
            let ann = 0;
            let top = +(totalPrice * monthlyPercentRate).toFixed(2);
            let bottom = +(1 - 1 / Math.pow(monthlyPercentRate + 1, numberOfMonth)).toFixed(2);
            ann = +(top / bottom).toFixed(0);
            const resultHtml = document.querySelector(".investement-calculator__count");
            resultHtml.innerHTML = ann;
            console.log(ann);
        };
        calculateBtn.addEventListener("click", getVal);
    }
    window["FLS"] = false;
    isWebp();
    formFieldsInit({
